    ...
    <div className={className}>
            ...
            <div className="aspect-square relative overflow-hidden rounded-lg bg-gray-100">
              ...
              {hoveredId === design.id && selectedId !== design.id && (
                <div className="absolute inset-0 bg-white/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="text-gray-900 text-sm font-semibold">Click to select</div>
                </div>
              )}
            </div>
      ...
      {hasMore && (
        <div className="mt-8 flex justify-center">
          <button
            onClick={onLoadMore}
            disabled={loading}
            className="px-6 py-3 bg-indigo-600 text-gray-900 rounded-lg hover:bg-indigo-700 disabled:bg-gray-500 disabled:cursor-not-allowed transition-colors duration-300"
          >
            {loading ? (
      ...
      {designs.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-900 mb-2">No designs found</h3>
